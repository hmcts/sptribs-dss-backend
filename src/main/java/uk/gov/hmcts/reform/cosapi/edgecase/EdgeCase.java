...
@Component
public class EdgeCase implements CCDConfig<CaseData, State, UserRole> {

    @Override
    public void configure(final ConfigBuilder<CaseData, State, UserRole> configBuilder) {
        configBuilder.setCallbackHost(System.getenv().getOrDefault("CASE_API_URL", "http://localhost:4550"));
        configBuilder.caseType(CommonConstants.CASE_TYPE, "New edge case", "Handling of edge cases");
        configBuilder.jurisdiction(CommonConstants.JURISDICTION, "Prl jurisdiction adoption", "edge-cases");
